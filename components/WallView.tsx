...
const WallView: React.FC<WallViewProps> = ({ 
  ...
  const findSmartSlot = (posts: PostType[]) => {
    ...
    const maxX = Math.max(...posts.map(p => p.x + POST_W));
  ...
  const handlePostSubmit = async (data: Partial<PostType>) => {
    ...
    if (containerRef.current) {
        const containerW = containerRef.current.clientWidth, containerH = containerRef.current.clientHeight;
        const targetX = slot.x + 150, targetY = slot.y + 125;
        setPan({ x: (containerW / 2) - (targetX * zoom), y: (containerH / 2) - (targetY * zoom) });
    }
  ...
  return (
    ...
    >
      ...
      <div className="fixed bottom-10 left-10 z-[100] bg-white/90 backdrop-blur-md p-2 rounded-2xl shadow-2xl flex items-center gap-2 border border-slate-200">
        <button onClick={() => handleManualZoom(-1)} className="p-2 hover:bg-slate-100 rounded-lg text-slate-600 transition-colors"><ZoomOut size={20} /></button>
        <span className="text-[10px] font-black text-slate-400 w-12 text-center">{Math.round(zoom * 100)}%</span>
        <button onClick={() => handleManualZoom(1)} className="p-2 hover:bg-slate-100 rounded-lg text-slate-600 transition-colors"><ZoomIn size={20} /></button>
      ...
      {showShareOverlay && (
        <div className="fixed inset-0 z-[300] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200 modal-overlay" onClick={() => setShowShareOverlay(false)}>
            <div className="bg-white rounded-[2.5rem] shadow-2xl p-8 max-w-sm w-full text-center space-y-6 relative transform transition-all scale-100" onClick={e => e.stopPropagation()}>
                <button onClick={() => setShowShareOverlay(false)} className="absolute top-6 right-6 p-2 hover:bg-slate-100 rounded-full text-slate-400 transition-colors"><X size={24} /></button>
                ...
                <div className="bg-white p-4 rounded-3xl border-2 border-cyan-100 shadow-[0_0_40px_-10px_rgba(8,145,178,0.2)] inline-block">
                    <img src={`https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(wallDeepLink)}&color=0891b2&bgcolor=ffffff`} alt="QR Code" className="w-48 h-48 rounded-xl object-contain"/>
                </div>
                ...
                <div className="flex gap-2">
                    <button onClick={handleCopyLink} className="flex-1 py-4 bg-white border border-slate-200 rounded-2xl shadow-sm hover:bg-slate-50 active:bg-slate-100 transition-all flex items-center justify-center gap-2 group">
                ...
                {isTeacher && (
                    <button onClick={handleClassroomShareOpen} className="w-full py-4 bg-white border border-slate-200 rounded-2xl shadow-sm hover:bg-slate-50 active:bg-slate-100 transition-all flex items-center justify-center gap-3 group">
      ...
      {showEditor && (
        <PostEditor 
          ...
          onClose={() => { setShowEditor(false); setEditingPostId(null); }} 
                ...
                <div className="grid grid-cols-[auto_1fr] gap-4">
                            ...
                            {showEmojiPicker && (
                                <div className="absolute top-full mt-2 left-0 z-50 bg-white rounded-2xl shadow-xl border border-slate-100 p-4 w-72 h-80 flex flex-col">
                                    <div className="mb-2 relative">
                                        <input type="text" placeholder="Search..." value={emojiSearch} onChange={(e) => setEmojiSearch(e.target.value)} className="w-full pl-8 pr-2 py-2 bg-slate-50 rounded-lg text-sm outline-none focus:ring-2 focus:ring-cyan-100" />
                                        <Search size={14} className="absolute left-2.5 top-2.5 text-slate-400" />
                                    </div>
                                    <div className="flex-1 overflow-y-auto custom-scrollbar grid grid-cols-5 gap-2 content-start">
                                        {filteredEmojis.map((emoji, idx) => (
                                            <button key={idx} onClick={() => { setSettingsForm({...settingsForm, icon: emoji}); setShowEmojiPicker(false); }} className="h-12 w-12 flex items-center justify-center rounded-xl hover:bg-slate-100 text-2xl transition-all hover:scale-110">{emoji}</button>
                                        ))}
                                    </div>
                                </div>
                            )}
                    ...
                    <div className="space-y-4">
                         <div className="space-y-2">
                            ...
                            <input type="text" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-900 outline-none focus:ring-2 focus:ring-cyan-500/20" value={settingsForm.name || ''} onChange={(e) => setSettingsForm({...settingsForm, name: e.target.value})} />
                         </div>
                         <div className="space-y-2">
                            <label className="text-xs font-bold text-slate-500 ml-1">Description</label>
                            <input type="text" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-700 outline-none focus:ring-2 focus:ring-cyan-500/20" value={settingsForm.description || ''} onChange={(e) => setSettingsForm({...settingsForm, description: e.target.value})} />
                         </div>
                    </div>
                </div>
              </section>
              <section className="space-y-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {WALL_GRADIENTS.map(g => (
                    <button key={g} onClick={() => setSettingsForm({ ...settingsForm, background: g })} className={`h-20 rounded-2xl bg-gradient-to-br ${g} ${settingsForm.background === g ? 'ring-4 ring-cyan-500 ring-offset-2 scale-95' : 'hover:scale-105'} transition-all shadow-sm`} />
                  ))}
                </div>
